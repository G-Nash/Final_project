// Importing the classifiers for the k-Nearest Neighbors Algorithm
import {
  KNNImageClassifier
} from 'deeplearn-knn-image-classifier';
import * as dl from 'deeplearn';

//importing classifiers for svm algorithm
import { OVAMClassifier } from 'deeplearn';
...
class Main {
  constructor() {
    ...
    //new
    this.svm = new OVAMClassifier(words.length);
  }
  ...
  train(gestureIndex) {
    ...
    if (this.videoPlaying) {
      ...
      //new
      const tensorExample = image.reshape([1, IMAGE_SIZE * IMAGE_SIZE * 3]);
      this.svm.addExample(tensorExample, gestureIndex);
  ...
  predict() {
    ...
    if (this.elapsed > this.fpsInterval) {
      ...
      if (this.videoPlaying) {
        ...
        if (Math.max(...exampleCount) > 0) {
          const knnPrediction = this.knn.predictClass(image);
          const svmPrediction = this.svm.predictClass(tensorExample);
  
          Promise.all([knnPrediction, svmPrediction])
            .then(([knnRes, svmRes]) => {
              // Combine kNN and SVM predictions
              const combinedPrediction = this.combineKNNAndSVM(knnRes, svmRes);
  
              for (let i = 0; i < words.length; i++) {
                if (
                  combinedPrediction.classIndex === i &&
                  combinedPrediction.confidences[i] > confidenceThreshold &&
                  combinedPrediction.classIndex !== this.previousPrediction
                ) {
                  this.setStatusText("Status: Predicting!", "predict");
                  this.predictionOutput.textOutput(
                    words[i],
                    this.gestureCards[i],
                    combinedPrediction.confidences[i] * 100
                  );
                  this.previousPrediction = combinedPrediction.classIndex;
                }
              }
            })
  ...
  combineKNNAndSVM(knnRes, svmRes) {
    const combinedConfidences = [];
  
    for (let i = 0; i < words.length; i++) {
      // Simple average of kNN and SVM confidences
      //const avgConfidence = (knnRes.confidences[i] + svmRes.confidences[i]) / 2;
      const avgConfidence = svmRes.confidences[i];
      combinedConfidences.push(avgConfidence);
    }
  
    const maxConfidenceIndex = combinedConfidences.indexOf(Math.max(...combinedConfidences));
  
    return {
      classIndex: maxConfidenceIndex,
      confidences: combinedConfidences,
    };
  }
